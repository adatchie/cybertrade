            ...
            for (const jan of uniqueJans) {
                try {
                    ...
                    if (metaResult) {
                        ...
                        // If no result found, but we have garbage title, clean it up
                        const itemsToUpdate = items.filter(i => i.janCode === jan);
                        itemsToUpdate.forEach(item => {
                            const garbage = ['買取Wiki', 'ゲーム機の買取専門店', '検索結果'];
                            if (garbage.some(g => item.name.includes(g))) {
                                InventoryService.update(item.id, {
                                    name: `Item ${item.janCode}`, // Reset to generic
                                    imageUrl: item.imageUrl // Keep image if any, or maybe clear it?
                                });
                                updatedCount++;
                            }
                        });
